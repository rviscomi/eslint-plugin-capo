...
export class HtmlEslintAdapter {
  ...
  matches(node, selector) {
    if (!node || !this.isElement(node)) {
      return false;
    }
    // Simple selector matching for now, as required by Capo
    // Capo mainly uses tag names and attribute selectors
    const tagName = this.getTagName(node);
    if (selector === tagName) {
      return true;
    }
    // Add more complex matching if needed, but this covers Capo's current needs
    return false;
  }
