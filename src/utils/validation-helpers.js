...
export function validateCSP(node, context) {
  ...
  if (httpEquiv?.toLowerCase() === 'content-security-policy') {
    warnings.push('CSP meta tags disable the preload scanner due to a bug in Chrome. Use the CSP header instead. Learn more: https://crbug.com/1458493');
  }
  ...
  if ('report-uri' in directives) {
    warnings.push('The report-uri directive is not supported. Use the Content-Security-Policy-Report-Only HTTP header instead.');
  }
  if ('frame-ancestors' in directives) {
    warnings.push('The frame-ancestors directive is not supported. Use the Content-Security-Policy HTTP header instead.');
  }
  ...
  switch (type) {
    ...
    case 'x-dns-prefetch-control':
      if (content === 'on') {
        ...
        warnings.push(`This is a non-standard way of disabling DNS prefetching, which is a performance optimization. Found content="${content}". Use content="off" if you have a legitimate security concern, otherwise remove it.`);
      } else {
        warnings.push('This is non-standard, however most browsers support disabling speculative DNS prefetching. It should still be noted that DNS prefetching is a generally accepted performance optimization and you should only disable it if you have specific security concerns.');
      }
      break;
...
export function validateMetaViewport(node) {
  ...
  if ('user-scalable' in directives) {
    ...
    if (userScalable === 'no' || userScalable === '0') {
      warnings.push(`Disabling zooming can cause accessibility issues to users with visual impairments. Found "${userScalable}".`);
    }
  ...
  if ('shrink-to-fit' in directives) {
    warnings.push('The shrink-to-fit directive has been obsolete since iOS 9.2. See https://www.scottohara.me/blog/2018/12/11/shrink-to-fit.html');
  }
...
export function validateContentType(node) {
  ...
  if (charset && charset.toLowerCase() !== 'utf-8') {
    warnings.push(`Documents are required to use UTF-8 encoding. Found "${charset}". Learn more: https://html.spec.whatwg.org/multipage/semantics.html#character-encoding-declaration`);
  }
